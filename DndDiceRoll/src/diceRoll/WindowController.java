...
public class WindowController {
    ...
    //Took way too long to realize that I could just init all of these in one line.
    @FXML
    ...
    public void tabResizeHbDiceContentResize(HBox DiceContainer) {
    	...
    	if (width == 399){
    		...
    		timeline.getKeyFrames().add(new KeyFrame(Duration.ZERO, new KeyValue(DiceContainer.prefWidthProperty(),399)));
            timeline.getKeyFrames().add(new KeyFrame(Duration.seconds(.6), new KeyValue(DiceContainer.prefWidthProperty(),800)));

                
    		timeline.play();
    	}else if(width > 399){
    		Timeline timeline = new Timeline();
    		timeline.getKeyFrames().add(new KeyFrame(Duration.ZERO, new KeyValue(DiceContainer.prefWidthProperty(),800)));
            timeline.getKeyFrames().add(new KeyFrame(Duration.seconds(.6), new KeyValue(DiceContainer.prefWidthProperty(),399)));
    ...
    //This block of methods is not good. There is def a better way to do this and I'm sorry for this.
    @FXML
